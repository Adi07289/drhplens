...
status: executing
last_updated: "2026-07-19T06:22:05.000Z"
progress:
  total_phases: 6
  completed_phases: 3
  total_plans: 36
...
Plan: 05-06 complete (Wave 5 — global metrics + precompute CLI + MLflow) → next 05-08 (regime/DRHP/anchor feature families) / 05-09 (baselines + DM gate) / 05-10 (model card) / 05-11 (live build)
**Status:** Ready to execute the next Wave 5/6 plan (05-08)
**Progress:** [█████████░] 85%
...
**Phase 5 Wave 5 metrics + precompute (05-06) COMPLETE** — the model→record bridge is built and offline-green: `pipelines/forecast/diagnostics.py::global_metrics` computes the GLOBAL coverage/MAE/per-year-RMSE ONCE (RAW coverage, never clamped to 0.80 — P17; abstain/band-less rows excluded; no fabricated year), and `pipelines/forecast/precompute.py::precompute_forecasts` runs `build_features → walk_forward → global_metrics` ONCE and writes one `data/forecasts/<id>.json` per catalogue IPO (own OOS band D5-11 or first-class abstain + the identical global metrics block D5-12 + as-of/OOS provenance + pinned `model_version`), fraction→percentage-points at the boundary (`RETURN_TO_PCT`), allow-list-gated write, per-IPO isolation (P14), and the single computation wrapped in an `mlflow.start_run()` on the local file backend (`mlruns/`, `MLFLOW_ALLOW_FILE_STORE` opt-in for MLflow 3.x). CODE-NOW-DEFER: proven OFFLINE by monkeypatching `walk_forward` (MLflow → tmp); the REAL committed records + `mlruns/` regenerate from the live panel at **05-11**. **Next: 05-08** (regime/DRHP/anchor feature families b/c/d on the same `X` contract), **05-09** (four baselines + Diebold–Mariano release gate + `min_train`/`cal_frac` tuning, D5-09), **05-10** (model card — calibration/PIT/SHAP over the walk-forward output), **05-11** (live-panel build: regenerate real records + `mlruns/`, close FCAST-01/FCAST-04). **FCAST-04 stays Pending** (real records/metrics land 05-11; the precompute is offline-fixture-proven only). Prior context:

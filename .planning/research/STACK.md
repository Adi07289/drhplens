# Stack Research — DRHPLens

**Domain:** Agentic RAG + ML over Indian-IPO DRHP/RHP prospectuses, with peer-fundamentals comparison and historical-IPO listing-day forecasting. Free/public-data, single-DS portfolio web app.
**Researched:** 2026-05-28
**Overall confidence:** HIGH on RAG/agent/eval stack and PDF parsing; MEDIUM on India-specific data libraries (active maintenance is the risk); HIGH on forecasting/uncertainty libraries; HIGH on free-tier deployment paths.
...
```
Frontend:      Streamlit 1.x (v1)  →  Next.js 15 + FastAPI (v2 polish)
Agent core:    LangGraph 1.2  (orchestration; cycles, state, HITL)
RAG core:      LlamaIndex 0.14 (ingestion, indices, query engines)
PDF parsing:   Docling 2.95 + pdfplumber 0.11 fallback; PyMuPDF for raw speed
Embeddings:    BAAI/bge-m3 (open) primary; voyage-3-large or Gemini text-embed for prod headroom
Reranker:      BAAI/bge-reranker-v2-m3 (open, Apache-2.0)
Vector DB:     Qdrant 1.18 (self-host docker; pgvector if Postgres present)
Hybrid search: BM25 (rank_bm25 or Qdrant sparse vectors) + dense + RRF
LLM:           Gemini 2.5 Flash + Groq Llama-3.3-70B (free tiers, router pattern)
Struct extract:Instructor 1.15 over Pydantic v2 schemas
DRHP fetch:    requests/httpx + chittorgarh + SEBI/BSE/NSE archive scraping
Prices/peers:  yfinance (.NS/.BO) + jugaad-data + screener.in scraping
ML/forecasting:scikit-learn 1.x + XGBoost 2.x + MAPIE (conformal intervals)
RAG evals:     RAGAS 0.4 (faithfulness/recall) + DeepEval (CI metrics) + Langfuse (tracing)
Experiments:   MLflow (local, free) for forecasting; W&B free tier optional
Deploy:        Hugging Face Spaces (free CPU, 16GB RAM) + Qdrant on Fly.io free
```

**One-line philosophy:** open-source-first, instrumented-by-default, free-tier deployable, every component swappable for a paid managed equivalent if/when v2 scales.
...
uv pip install --group dev \
  "pytest" "ruff" "mypy" "pre-commit" "rich" "typer"
```
...
**If embedding latency is the bottleneck (offline indexing of 50+ DRHPs):**
...
**If a specific DRHP defeats Docling table extraction:**
...
**If you need long-context single-shot summarization (e.g. "the whole risk factor section"):**
...
**If `yfinance` returns stale or missing data for a fresh listing:**
...
**If Streamlit becomes constraining (v2):**
...
**Listing-return label construction (concrete):**
Listing-day return = `(listing_day_close / issue_price) - 1`, where `issue_price` is the upper end of the price band per RHP and `listing_day_close` is from yfinance/jugaad-data. Construct labels for 2014→present (~800–1000 mainboard IPOs) — this matches the dataset size in the 2025 India IPO ML literature.

---

## RAG Stack Specifics for DRHP Documents

**Chunking strategy (informed by 2025 financial-RAG research):**
...
**Retrieval pipeline:**
1. **Hybrid**: BM25 (sparse) + bge-m3 (dense) → top-50 candidates per leg → RRF fusion → top-30.
2. **Rerank**: bge-reranker-v2-m3 → top-5.
3. **Generate** with citations: Gemini 2.5 Flash, prompt-constrained to cite `(DRHP §X, p.Y)` for every claim.

**Evaluation (the artifact a recruiter will read):**
...
**Hard limit to design for:** HF Spaces only writes to `/tmp` — keep the vector DB *out of process* (use Qdrant Cloud or a remote Qdrant), not embedded.

---

## Sources

**Library docs / PyPI (versions verified May 2026):**
...
---
*Stack research for: DRHPLens — Indian-IPO agentic RAG portfolio web app*
*Researched: 2026-05-28*
